...
class TestShellIntegration(TestCase):
    def setUp(self):
        """Активируем фейковую файловую систему"""
        self.setUpPyfakefs()
        self.shell = Shell()

    '''def tearDown(self):
        """Очищаем логгер"""
        if hasattr(self, 'shell') and hasattr(self.shell, 'logger'):
            for handler in self.shell.logger.handlers:
                handler.close()
                self.shell.logger.removeHandler(handler)'''
    ...
    def test_ls_basic(self):
        ...
        output = self.capture_output(lambda: self.shell.execute_ls([], []))
    ...
    def test_cd_and_pwd(self):
        ...
        start_dir = os.getcwd()
        self.shell.execute_cd([], ['subdir'])
        cur_dir = self.capture_output(lambda: self.shell.execute_pwd([],[]))
        cur_dir = cur_dir.strip()
        self.assertTrue(cur_dir.endswith('subdir'))
        self.shell.execute_cd([], ['..'])
        cur_dir = self.capture_output(lambda: self.shell.execute_pwd([],[]))
        cur_dir = cur_dir.strip()
    ...
    def test_touch_and_cat(self):
        ...
        self.shell.execute_touch([], ['test_file.txt'])
        self.assertTrue(os.path.exists('test_file.txt'))
        with open('test_file.txt', 'w') as f:
            f.write('hello world\nline two')
        output = self.capture_output(lambda: self.shell.execute_cat([], ['test_file.txt']))
    ...
    def test_cp_file(self):
        ...
        self.shell.execute_cp([], ['source file.txt', 'dest file.txt'], False)
    ...
    def test_mv_file(self):
        ...
        self.shell.execute_mv([], ['old_name.txt', 'new_name.txt'], False)
    ...
    def test_rm_file(self):
        ...
        with mock.patch('builtins.input', return_value='y'):
            self.shell.execute_rm([], ['to_delete.txt'], False)
    ...
    def test_grep_in_file(self):
        """Тест поиска в файле"""
        self.fs.create_file('grep test.txt',
                           contents='first line\npattern here\nthird line\nanother pattern')

        output = self.capture_output(lambda:
            self.shell.execute_grep([], ['pattern', 'grep test.txt']))
        self.assertIn('pattern here', output)
        self.assertIn('another pattern', output)
        self.assertNotIn('first line', output)

        self.fs.create_file('grep test 2.txt',
                           contents='first line\npatTern here\nthird line pattern\nanother paTTern')

        output = self.capture_output(lambda:
            self.shell.execute_grep(['-i'], ['pattern', 'grep test 2.txt']))
        self.assertIn('patTern here', output)
        self.assertIn('another paTTern', output)
        self.assertIn('third line', output)
        self.assertNotIn('first line', output)

        self.fs.create_file('grep test 3.txt',
                           contents='first line\npatTern here\nthird line pattern\nanother paTTern')

        output = self.capture_output(lambda:
            self.shell.execute_grep([], ['pattern', 'grep test 3.txt']))
    ...
    def test_undo_rm(self):
        ...
        with mock.patch('builtins.input', return_value='y'):
            self.shell.execute_rm([], ['undo_test.txt'], False)
        self.assertFalse(os.path.exists('undo_test.txt'))
        self.shell.execute_undo([], [])
    ...
    def test_undo_cp_with_overwrite(self):
        ...
        self.shell.execute_cp([], ['source.txt', 'target.txt'], False)
    ...
    def test_undo_mv_with_overwrite(self):
        ...
        self.fs.create_file('source.txt', contents='source content')
        self.fs.create_file('target.txt', contents='target content')
        self.shell.execute_mv([], ['source.txt', 'target.txt'], False)
    ...
    def test_undo_multiple_operations(self):
        """Тест отмены нескольких операций в обратном порядке"""
        self.fs.create_file('file1.txt', contents='content1')
        self.fs.create_file('file2.txt', contents='content2')

        self.shell.execute_cp([], ['file1.txt', 'copy.txt'], False)

        with mock.patch('builtins.input', return_value='y'):
            self.shell.execute_rm([], ['file2.txt'], False)

        self.shell.execute_mv([], ['file1.txt', 'moved.txt'], False)
    ...
    def test_rm_nonexistent_file(self):
        ...
        output = self.capture_output(lambda:
            self.shell.execute_rm([], ['nonexistent.txt'], False))
    ...
    def test_cp_nonexistent_source(self):
        ...
        output = self.capture_output(lambda:
            self.shell.execute_cp([], ['nonexistent.txt', 'target.txt'], False))
        self.assertIn("FileNotFoundError", output)
        self.assertIn("doesn't exist", output)
    ...
    def test_mv_nonexistent_source(self):
        ...
        output = self.capture_output(lambda:
            self.shell.execute_mv([], ['nonexistent.txt', 'target.txt'], False))
        self.assertIn("FileNotFoundError", output)
        self.assertIn("doesn't exist", output)
    ...
    def test_cd_nonexistent_directory(self):
        ...
        output = self.capture_output(lambda:
            self.shell.execute_cd([], ['nonexistent_dir']))
        self.assertIn("FileNotFoundError", output)
        self.assertIn("doesn't exist", output)
    ...
    def test_cat_nonexistent_file(self):
        ...
        output = self.capture_output(lambda:
            self.shell.execute_cat([], ['nonexistent.txt']))
        self.assertIn("FileNotFoundError", output)
        self.assertIn("doesn't exist", output)
    ...
    def test_cat_directory_instead_of_file(self):
        ...
        self.fs.create_dir('test_dir')
        output = self.capture_output(lambda:
            self.shell.execute_cat([], ['test_dir']))
    ...
    def test_invalid_command_syntax(self):
        ...
        output = self.capture_output(lambda:
            self.shell.execute_ls([], ['arg1', 'arg2']))
        self.assertIn("ShellSyntaxError", output)
        output = self.capture_output(lambda:
            self.shell.execute_cp([], ['only_one_arg'], False))
        self.assertIn("ShellSyntaxError", output)
    ...
    def test_rm_cancel_confirmation(self):
        ...
        with mock.patch('builtins.input', return_value='n'):
            self.capture_output(lambda:
                self.shell.execute_rm([], ['to_cancel.txt'], False))
        self.assertTrue(os.path.exists('to_cancel.txt'))

    def test_invalid_options(self):
        """Тест невалидных опций команд"""
        output = self.capture_output(lambda:
            self.shell.execute_ls(['--invalid-option'], []))
        self.assertIn("ShellSyntaxError", output)
        output = self.capture_output(lambda:
            self.shell.execute_cp(['-x'], ['src.txt', 'dst.txt'], False))
        self.assertIn("ShellSyntaxError", output)

    def test_zip_unzip_nonexistent(self):
        """Тест архивации несуществующих файлов"""
        output = self.capture_output(lambda:
            self.shell.execute_zip([], ['nonexistent', 'archive.zip']))
        self.assertIn("FileNotFoundError", output)
        self.assertIn("doesn't exist", output)
        output = self.capture_output(lambda:
            self.shell.execute_unzip([], ['nonexistent.zip']))
        self.assertIn("FileNotFoundError", output)
        self.assertIn("doesn't exist", output)
    ...
    def test_grep_in_nonexistent_file(self):
        ...
        output = self.capture_output(lambda:
            self.shell.execute_grep([], ['pattern', 'nonexistent.txt']))
        self.assertIn("FileNotFoundError", output)
        self.assertIn("doesn't exist", output)
